...
<!-- Add Artist Form -->
<form action="{{ route('makeup-artists.store') }}" method="POST">
    @csrf
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="{{ $artist->available ? 'available' : 'not-available' }}">
                {{ $artist->available ? 'Yes' : 'No' }}
            </td>
            <td>{{ $artist->booked_appointments }}</td>
            <td><a  style="font-size:25px;"  class="fa-solid fa-trash-can text-danger" href="{{url('delete_artist' , $artist->id)}}"></a></td>
        </tr>
        @endforeach
    </tbody>
</table>
  ...
  <script>
    ...
    alert('Appointment booked for stylist ID: ' + id);
}

function cancelAppointment(id) {
    alert('Appointment canceled for stylist ID: ' + id);
}
</script>
